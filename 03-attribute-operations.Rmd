...
world_agg5 = world %>% 
  ...
  mutate(Densité = round(Pop / Area)) %>%     # calcule la densité de population
...
knitr::kable(
  world_agg5,
  caption = "Les 3 continents les plus peuplés classés par densité de population (personnes par km carré).",
...
Plus de détails sont fournis dans les pages d´aide (qui sont accessibles via `?summarize` et `vignette(package = "dplyr")` et le chapitre 5 de 5 of [R for Data Science](http://r4ds.had.co.nz/transform.html#grouped-summaries-with-summarize). 
```
...
Heureusement, les tableau de données non spatiaux avec une colonne de liste de géométrie (comme `coffee_world`) peuvent être converties en un objet `sf` comme suit : `st_as_sf(coffee_world)`. 
```
